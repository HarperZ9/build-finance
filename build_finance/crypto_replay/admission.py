...
def admit_local_fixture(captured: CapturedFixture) -> AdmissionBatch:
    ...
    batch_reasons: list[str] = [*fixture_reasons, *rights.reason_codes, *universe.reason_codes]
    records: list[bytes] = []
    admitted_candidates: list[ParsedJupiterFixture] = []

    for captured_file in captured.files:
        parsed = _parse_or_none(captured_file)
        file_reasons = _file_reason_codes(captured_file)
        profile_reasons = _profile_reason_codes(captured, captured_file, parsed, universe)
        raw_reasons = (*fixture_reasons, *rights.reason_codes, *universe.reason_codes, *file_reasons, *profile_reasons)
...
def _universe_evidence(manifest: Mapping[str, JsonValue] | None) -> _UniverseEvidence:
    ...
    if not isinstance(allowed_markets_value, list):
        reasons.append("ADMISSION_UNIVERSE_BIASED")
    else:
        for row in allowed_markets_value:
            ...
            if market_id is None or base_mint is None or quote_mint is None:
                reasons.append("ADMISSION_UNIVERSE_BIASED")
                continue
...
def _profile_reason_codes(
    ...
    reasons: list[str] = []
    manifest = captured.manifest if isinstance(captured.manifest, Mapping) else {}
    if _string_or_none(manifest.get("venue_profile")) != _PROFILE_VENUE:
